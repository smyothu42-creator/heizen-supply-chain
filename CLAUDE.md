...
The masthead carries Heizen's own mark — see *The wordmark* below.
...
| **Research** | **Live, in active use** | Company dossier. Six directions, Brief / Full toggle |
| **Gaps** | Live | Problems Heizen can solve, what each needs first, and a plan with dates. **Not priced. See *Gaps has no money on it*.** |
...
component and one prop; see *Gaps has no money on it*, and read the differences
noted inline below.
...
node carries three axes* under the Operations notes.
...
| **Chat is a side panel** | Settled. `AiPanel`, triggered from the masthead beside the theme control, pushing the page from `lg` and overlaying below it. Popover and contextual click-to-ask were the alternatives; see *The assistant*. |
...
| Styling | Tailwind v4 with semantic tokens in `src/app/globals.css`, themed to the reference dashboard — see below |
| shadcn/ui | **Installed**, on request, so the prototype reads as production rather than as a one-off. `components.json` (new-york, cssVariables), primitives in `src/components/ui`, Radix + `class-variance-authority` + `lucide-react` in `package.json`. See *The design system is shadcn's now*. |
...
**Light is the default and the only automatic theme.** The
`prefers-color-scheme: dark` block is gone from `globals.css` and the
`matchMedia` fallback is gone from `ThemeToggle`: dark is opt-in through
`[data-theme="dark"]`, which the toggle sets and the inline script in
`layout.tsx` restores before first paint. A tool demoed to investors on an
unknown machine should not pick its palette from that machine's OS setting, and
dark mode here exists so a designer can check the health colours rather than as
a preference to be guessed at.
...
- **`ThemeToggle` takes a `tone`.** It lives on the masthead in the app and on the
  page on `/`. Masthead greys are ~2:1 on ivory, so one set of tokens cannot
  ...
  **It is hidden, not removed**, on request: `const SHOW = false` at the top of
  the file and an early `return null` *after* the hooks. One flag rather than
  two commented-out call sites, because the two call sites are what drift — a
  control taken out of `AppShell` and left on `/` is how a product ends up with
  a theme you can reach from one screen and not the next. Flipping it back is
  the whole change.

  **Dark mode still works and is still checked.** `[data-theme="dark"]` is what
  switches the palette, the inline script in `layout.tsx` still restores a
  stored choice, and both harnesses set the attribute directly rather than
  pressing the button — so `check:ui` still measures contrast in both themes and
  `pnpm shots` still writes a dark screenshot of every surface. **Hiding the
  control did not make the second palette unverified**, which is the thing that
  would otherwise rot while nobody could see it.
